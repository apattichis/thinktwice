...
export default function Home() {
  const { apiKey, setApiKey, clearApiKey, hasKey, hydrated } = useApiKey();
  const { state, run } = usePipeline(apiKey);
  ...
  // Don't render until sessionStorage is hydrated (prevents flash)
  if (!hydrated) return null;

  return (
    <div className="bg-bg-primary" style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <Header
        showHowItWorks={hasKey && !hasStarted}
        onHowItWorks={() => setShowHowItWorks(true)}
        hasApiKey={hasKey}
        onRemoveKey={clearApiKey}
      />
      ...
      <main style={{ paddingTop: "80px", paddingBottom: "64px", flex: 1 }}>
        <div style={{ maxWidth: "720px", margin: "0 auto", padding: "0 24px" }}>
          <AnimatePresence mode="wait">
            {!hasKey ? (
              <ApiKeyGate key="gate" onKeyValid={setApiKey} />
            ) : (
              <motion.div
                key="app"
                initial={{ opacity: 0 }}
